# Hi, I'm Olivia 👋

**Informatics Student · Data Science · Machine Learning · Full-Stack Development**  
📍 Vienna, Austria &nbsp;|&nbsp; 🎓 IMC University of Applied Sciences, Krems &nbsp;|&nbsp; 🌍 Open to opportunities worldwide

---

## About Me

I'm a fourth-semester Informatics student specializing in Data Science and Machine Learning, with hands-on experience across the full stack — from training RL agents and building forecasting models to shipping full-stack web applications. Planning on switching to quantitative finance based roles.

Currently:
- 🔬 Writing my bachelor's thesis on **Financial Forecasting** with sentiment analysis integration
- 🤖 Completing a virtual **ML/AI internship** focused on bot detection and customer interaction modeling
- 📦 Incoming intern at **Amazon, Berlin** (September 2026)

I care about building things that are both technically sound and practically useful — whether that's a reinforcement learning agent navigating a custom game environment or a Spotify-integrated music player with real-time playback.

---

## 🛠️ Tech Stack

**Languages**  
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=flat&logo=c-sharp&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

**ML / Data Science**  
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)

**Frontend & Backend**  
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![ASP.NET](https://img.shields.io/badge/ASP.NET_Core-512BD4?style=flat&logo=dotnet&logoColor=white)
![Pygame](https://img.shields.io/badge/Pygame-000000?style=flat&logo=python&logoColor=white)

**Tools & DevOps**  
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat&logo=figma&logoColor=white)
![PowerBI](https://img.shields.io/badge/PowerBI-F2C811?style=flat&logo=powerbi&logoColor=black)

---

## 📂 Featured Projects

| Project | Description | Stack |
|---|---|---|
| [🎮 Reinforcement Learning Game](https://github.com/Iminza67/ReinforcementLearning) | Custom adventure game where a Q-Learning agent learns to navigate from start to goal while avoiding traps | Python, Pygame, Q-Learning, Jupyter |
| [🎵 Spotify Music Player](https://github.com/Iminza67/SpotifyClone) | Full-stack music streaming app with real-time Spotify playback, OAuth 2.0, and playlist management | React, TypeScript, Node.js |
| [🎬 Movie Database App](https://github.com/Iminza67/movie-database) | Full-stack CRUD app with MVC architecture, relational DB design, and responsive UI | Node.js, SQL, Bootstrap |

---

## 🌐 Languages

🇬🇧 English (C2) &nbsp;·&nbsp; 🇩🇪 German (B2) &nbsp;·&nbsp; 🇰🇪 Swahili (C2, native) &nbsp;·&nbsp; 🇨🇳 Mandarin (beginner, in progress)

---

## 📫 Get in Touch

[![Email](https://img.shields.io/badge/Email-oliviahamisi@gmail.com-D14836?style=flat&logo=gmail&logoColor=white)](mailto:oliviahamisi@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-oliviahamisi.dev-1A56DB?style=flat&logo=globe&logoColor=white)](https://oliviaportfolio.netlify.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-oliviahamisi-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/oliviahamisi)
